...
func NewServerMux(store Store, logger Logger) (*ServerMux, error) {
	...
	router.HandleFunc("/books", api.addBook).Methods("POST")
	router.HandleFunc("/books", api.updateBook).Methods("PUT")
	router.HandleFunc("/books/{id}", api.deleteBooks).Methods("DELETE")
