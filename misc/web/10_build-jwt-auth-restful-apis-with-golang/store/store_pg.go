...
func (s *StorePg) Signup(email, password string) (int, error) {
	id := 0
...
func (s *StorePg) Login(email_ string) (id int, email string, password string, err error) {
	err = s.db.QueryRow("select id, email, password from users where email = $1", email_).
