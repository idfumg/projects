...
func (app *application) render(w http.ResponseWriter, r *http.Request, t string, d *TemplateData) error {
	...
	d.IP = app.ipFromContext(r.Context())
...
func (app *application) authenticate(r *http.Request, user *data.User, password string) bool {
	...
	log.Println("authenticate pic filename:", user.ProfilePic.FileName)
...
func (app *application) UploadFiles(r *http.Request, uploadDir string) ([]*UploadedFile, error) {
	...
	const size = int64(1024 * 1024 * 5)
	err := r.ParseMultipartForm(size)
	if err != nil {
		return nil, fmt.Errorf("file too big and must be less than %d", size)
	}
	...
	for _, headers := range r.MultipartForm.File {
		for _, header := range headers {
			uploadedFiles, err = func(uploadedFiles []*UploadedFile) ([]*UploadedFile, error) {
				...
				if outfile, err = os.Create(path.Join(uploadDir, uploadedFile.OriginalFilename)); err != nil {
					return nil, err
				} else {
